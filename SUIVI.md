...
- **PROCHAINE ETAPE : F3** = mapping pays×categorie×devise×couche (ancre sur ref_indices_fundafrica) + plan de schema (table series par indice : indicator_code, scope national/converted/africa, devise, valeur, as_of_date, source, is_official, is_synthetic, status) — a soumettre a l'utilisateur AVANT toute implementation. Decisions en attente : CEMAC (source ?), 337 fonds dormants (politique ?).
