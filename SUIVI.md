...
- **Bloc B1 (degel indices) pret, EN ATTENTE D'EXECUTION SSH** : diag cron+MONIA puis backfill MASI/NSE 06-26→07-09 via scrape_indices_daily --execute --date (idempotent, propagation indRef incluse).
...
- **F2 (matrice sources en ligne) EN ATTENTE** : les 2 agents web (Maroc+Tunisie, Nigeria+Afrique+BCE) ont echoue sur limite de session (reset 23h10 UTC) — a relancer.
