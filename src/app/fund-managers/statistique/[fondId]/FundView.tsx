...
export default function Fond() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const response = await fetch(`${urlconstant}/api/getSocietebyidstat/${id}`);
        if (response.ok) {
  ...
  const handleDeviseChange = async (e: { target: { value: any; }; }) => {
    ...
    const response = await fetch(`${urlconstant}/api/getSocietebyidstat/${id}?query=${selectedValue}`);
    if (response.ok) {
      const responseData = await response.json();
      settotalfonds(responseData.data.totalfonds)
      settotalfondsignore(responseData.data.totalfondsignore)
  ...
  options = {
    ...
    title: {
      text: "Graphique  de répartition par classe d&#39;actifs du nombre de fonds gérés"
    },
    series: [{
      name: 'Fonds',
  ...
  options1 = {
    ...
    title: {
      text: "Graphique de  répartition par classe d&#39;actifs des AUMs"
    },
    series: [{
      name: 'Fonds',
  ...
  options2 = {
    ...
    title: {
      text: 'Graphique de repartition par notation du nombre de fonds'
    },
    series: [{
      name: 'Fonds',
      data: seriesData.map(({ name, y }) => ({ name, y }))
  ...
  options3 = {
    ...
    title: {
      text: `Graphique du profil risque(Volatilité) / rendement (Perf Annualisée) des fonds gérés`
    },
    xAxis: {
      title: {
                ...
                <div className="box ">
                  <div className="box-body">
                    <div className="d-md-flex justify-content-between align-items-center">
                      <div className="col-2">
                        <p><span className="text-primary">Société de gestion</span> | <span className="text-fade"></span></p>
                    ...
                    <div className="row">
                      ...
                      <ul className="tabs-menu" style={{ display: 'flex', flexWrap: 'wrap', padding: '0', margin: '0', listStyleType: 'none', width: '100%' }}>
                        <li style={{ margin: '0', padding: '0', flex: '1' }}>
                          <Button
                            href={`/fund-managers/${id.replace(/ /g, '-')}`}
                            style={{
                              width: '100%',
