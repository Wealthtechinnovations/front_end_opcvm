...
interface Funds {
  data: {
    fonds: any[]; // ou un type spécifique pour les éléments du tableau 'funds'
  };
}
...
export default function Comparaison() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const mappedOptions3 = data3?.data?.societes.map((funds: any) => ({

          value: funds.nom,
          label: funds.nom, // Replace with the actual property name
          // Replace with the actual property name
      }));
        setOptionsSociete(mappedOptions3);

        const data2 = await getfonds();
        const mappedOptions2 = data2?.data?.funds.map((funds: any) => ({

          value: funds.value,
          label: funds.label, // Replace with the actual property name
          // Replace with the actual property name
      }));
        setFundsOptions(mappedOptions2);
  ...
  const handleSort = (key: string) => {
    ...
    if (funds && funds.data.fonds) {
      ...
      const sortedFundsCopy = [...funds.data.fonds];
      const filteredFunds = sortedFundsCopy.filter(item => item[sortConfig.key] !== '-');

      filteredFunds.sort((a, b) => {

        if (sortConfig.key == "nom_fond" || sortConfig.key == "pays" || sortConfig.key == "societe_gestion") {
      ...
      setSortedFunds({ data: { fonds: filteredFunds } });
      setFunds({ data: { fonds: filteredFunds } });
    }
  };
  ...
  return (
    ...
    < Fragment >
      ...
      <Head>
        ...
        <link rel="canonical" href={`${urlsite}/funds/search`} />
                            ...
                            <thead className="table-header">
                              <tr className="text-center">
                                {headers.tabAccueil.map((header) => (
                                  <th className="text-center" key={header.key} onClick={() => handleSort(header.key)} style={{ height: '35px',width: '20%' }}>
                                    {header.label}
                            ...
                            <tbody>
                              {funds?.data?.fonds.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item) => (
                                <tr className="text-center" key={item.id}>
                                  <td className="text-center"><Link href={`/funds/${generateFundSlug(item?.nom_fond || '', item?.code_ISIN || '', item.id)}`}>{item?.nom_fond}</Link></td>
                                  <td>{item.pays}</td>
