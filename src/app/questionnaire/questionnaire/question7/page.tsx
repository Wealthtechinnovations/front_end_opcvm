...
export default function Profile(props: PageProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const result = await fetch(`${urlstableconstant}/api/mifid/find-quiz-part-seven`, {
          headers: {
  ...
  const updatePartOne = async (event: { preventDefault: () => void; }) => {
    ...
    try {
      ...
      let categoryProfilePartSeven = "";
      let commentPartSeven = "";

      if (totalPoint >= -8 && totalPoint <= -4) {
        categoryProfilePartSeven = "Faible";
        commentPartSeven = "Vous n'avez pas ou peu de préférences en matière d'investissement. Vous n'avez aucun goût pour un secteur d'activité, un marché géographique, une conviction éthique ou sociale ou un style d'investissement. Vous n'avez probablement pas de plaisir ou de passion pour l'investissement. Vous devriez réfléchir à ce qui vous plaît, vous intrigue ou vous amuse. Vous devriez également vous informer sur les tendances et les opportunités du marché.";
      } else if (totalPoint >= -3 && totalPoint <= 3) {
        categoryProfilePartSeven = "Moyenne";
        commentPartSeven = "Vous avez des préférences modérées ou variées en matière d'investissement. Vous avez un goût pour un secteur d'activité, un marché géographique, une conviction éthique ou sociale ou un style d'investissement, mais pas tous à la fois ou pas tous au même niveau. Vous avez probablement un plaisir ou une passion pour l'investissement, mais pas de façon intense ou exclusive. Vous devriez clarifier vos préférences en matière d'investissement, en hiérarchisant vos goûts, vos intérêts, vos convictions et votre style. Vous devriez également vous renseigner sur les solutions adaptées à votre profil et à vos préférences.";
      } else if (totalPoint >= 4 && totalPoint <= 12) {
        categoryProfilePartSeven = "Elevée";
        commentPartSeven = "Vous avez des préférences importantes ou spécifiques en matière d'investissement. Vous avez un goût pour un secteur d'activité, un marché géographique, une conviction éthique ou sociale et un style d'investissement, tous présents et tous significatifs. Vous avez probablement un plaisir et une passion forts pour l'investissement, de façon vive et affirmée. Vous devriez affiner vos préférences en matière d'investissement, en tenant compte de votre situation actuelle et future, de votre profil de risque et de votre horizon de placement. Vous devriez également vous appuyer sur votre expérience et votre connaissance pour choisir les meilleurs placements.";
      }

      // Stockage dans le localStorage
      localStorage.setItem("categoryProfilePartSeven", categoryProfilePartSeven);
      localStorage.setItem("commentPartSeven", commentPartSeven);
      localStorage.setItem("totalPoint7", totalPoint);
      ...
      setTimeout(() => {
        router.push("/questionnaire/questionnaire/question8");
      }, 5000);
