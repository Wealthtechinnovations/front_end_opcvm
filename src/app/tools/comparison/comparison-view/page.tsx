...
export default function Comparaisonview() {
  ...
  const handleOkClick = () => {
    ...
    setTimeout(() => {
      const redirectUrl = `/tools/comparison/comparison-view?selectedRows=${concatenatedValues}`;
                            ...
                            <tbody>
                              {funds?.data?.funds.map((item) => (
                                <tr className="text-right" key={item.id}>
                                  ...
                                  <td className="text-left"><Link href={`/funds/${generateFundSlug(item.fundData?.nom_fond || "", item.fundData?.code_ISIN || "", item.fundData?.id)}`}>{item.fundData?.nom_fond}</Link></td>
                                    ...
                                    <tbody>
                                      {funds?.data?.funds.map((item) => (
                                        <tr className="text-right" key={item.id}>
                                          ...
                                          <td className="text-left"><Link href={`/funds/${generateFundSlug(item.fundData?.nom_fond || "", item.fundData?.code_ISIN || "", item.fundData?.id)}`}>{item.fundData?.nom_fond}</Link></td>
                                          <td className="text-left"><Link href={`/fund-managers/${item.fundData?.societe_gestion.replace(/ /g, '-')}`}>{item.fundData?.societe_gestion}</Link></td>
