                    ...
                    <div className="row">
                      ...
                      <form onSubmit={handleFormSubmit}>
                        ...
                        <div className=" text-center">
                          <button type="submit" style={{ width: '150px', backgroundColor: "#1B3A5C", color: "white" }} className="btn btn-main">Rechercher</button>
                        </div>
                      </form>
                      <br />
                      ...
                      <div className="row text-center">
                              ...
                              <tbody>
                                {funds?.data?.funds.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item) => (
                                  <tr className="text-center" key={item.id}>
                                    ...
                                    <td className="text-center"><Link href={`/funds/${generateFundSlug(item.fundData?.nom_fond || "", item.fundData?.code_ISIN || "", item.fundData?.id)}`}>{item.fundData?.nom_fond}</Link></td>
                                      ...
                                      <tbody>
                                        {funds?.data?.funds.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((item) => (
                                          <tr className="text-center" key={item.id}>
                                            ...
                                            <td className="text-center"><Link href={`/funds/${generateFundSlug(item.fundData?.nom_fond || "", item.fundData?.code_ISIN || "", item.fundData?.id)}`}>{item.fundData?.nom_fond}</Link></td>
                                            <td className="text-center"><Link href={`/fund-managers/${item.fundData?.societe_gestion.replace(/ /g, '-')}`}>{item.fundData?.societe_gestion}</Link></td>
                        ...
                        {selectedRows.length > 0 && (
                          <div className="text-left">
                            <Link
                              href={{
                                pathname: '/tools/comparison/comparison-view',
