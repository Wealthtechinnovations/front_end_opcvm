...
export async function generateStaticParams() {
    ...
    return data.paysOptions.map((fund: Fund) => ({
        paysId: fund.value.toString(), // Assurez-vous que le paramètre correspond à votre route dynamique
    }));
}

// Fonction pour générer les métadonnées pour le SEO
export async function generateMetadata({ params }: { params: { paysId: any } }) {
    const response = await fetch(`${urlconstant}/api/getpaysidmeta/${params.paysId.replace(/-/g, ' ')}`);
    const fund = await response.json();
    console.log(fund)
    console.log("fundaaa",params.paysId);
    ...
    return {
        ...
        canonical: `https://funds.chainsolutions.fr/pays/${fund.data.pays.pays}`,
    };
}
