...
export default function Vente() {
  ...
  const [entries, setEntries] = useState([
    { date: '', type: 'vente', quantite: 0, quantiteachat: 0, montant: 0, fondId: 0, portefeuilleselect: null },
  ]);
  const prevPage = () => {
    setFormData({ ...formData, page: formData.page - 1 });
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    const firstTransaction = transactionData
      //   .reverse()
      .find((transaction) => transaction.type === 'ajoutcash'); const isInvalidQuantity = entries.some(entry => entry.quantite === 0 || entry.quantiteachat > entry.quantite);
    const isInvalidQuantity1 = entries.some(entry => entry.quantiteachat > 0);
    const isInvalidQuantity2 = entries.some(entry => entry.montant > 0);
    const isInvalidQuantity3 = entries.some(entry => firstTransaction && entry.date > firstTransaction.date);

    if (!isInvalidQuantity3) {
      setError("La date de vente doit etre superieure à la date de depot")
    ...
    if (isInvalidQuantity) {
      setError("La quantité doit être supérieure à 0 et inférieure ou égale à la quantité existante")

      // Afficher un message d'erreur et ne pas poursuivre le traitement du formulaire
      console.error('Erreur : La quantité doit être supérieure à 0 et inférieure ou égale à la quantité existante.');
      return;
    }
    if (!isInvalidQuantity1) {
      setError("Au moins une quantité vendue doit être supérieure à 0.")
    ...
    Swal.fire({
      ...
      html: 'Vente en cours, ne pas quitter la page!...',
      ...
      fetch(`${urlconstant}/api/createtransactions`, {
        ...
        .then((data) => {
          ...
          if (data.code === 200) {
            ...
            setTimeout(() => {
              const href = `/panel/portfolio/portefeuillereconstitution?selectedfund=${allfund}&portefeuille=${selectedportfeuille}`;
              ...
              <div className="box ">
                <div className="box-body">
                  <div className="d-md justify-content-between align-items-center">
                    ...
                    <div>
                      <p><span className="text-primary">Vente d actif</span> | <span className="text-fade"></span></p>
                          ...
                          <div className="row">
                            ...
                            <div className="col-2">
                              ...
                              <input
                                ...
                                value={entry.quantiteachat}
                                onChange={(e) => {
                                  const updatedEntries = [...entries];
                                  updatedEntries[index].quantiteachat = parseInt(e.target.value);
                                  setEntries(updatedEntries);
                                }}
                              /></div>
                            {/* <div className="col-3">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const updatedEntries = [...entries];
                                  updatedEntries[index].montant = parseInt(e.target.value);
                                  setEntries(updatedEntries);
                                }}
                              /></div>*/}
                            <div className="col-3">
